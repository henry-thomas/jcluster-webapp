    ...
    transferConfigCharts: [
        {
            name: 'Energy Chart',
            cb: function () {
                cm.tranferChartConfig({toName: "Energy Chart", fromValue: "powerHistorySimple", toValue: "energyHistory1"});
            }},
        {
            name: 'Live Chart',
            cb: function () {
                cm.tranferChartConfig({toName: "Live Chart", fromValue: "powerHistorySimple", toValue: "liveChart1"});
            }}
    ]
};
var phc = {
...
//    selectedDate: moment("20201103", "YYYYMMDD").endOf('day'), // For testing purposes
    ...
    getPowerTypesFromDb() {
        chartUtilGetPowerTypeList([{
                name: "calbackFn",
                value: "phc.savePowerTypes"
            }]);
    },

    savePowerTypes: function (powerTypes) {
        let powerTypesList = JSON.parse(powerTypes);
        phc.powerTypeMap = {};
        powerTypesList.forEach((pt) => {
            if (pt.powerType !== 'other') {
                if (!phc.powerTypeMap[phcUtil.getReadablePowerName(pt.powerType)]) {
                    phc.powerTypeMap[phcUtil.getReadablePowerName(pt.powerType)] = [];
                }
                phc.powerTypeMap[phcUtil.getReadablePowerName(pt.powerType)].push(pt.powerName);
            }
        });
            ...
            if (Array.isArray(powerData)) {
                for (var i = 0; i < powerData.length; i++) {
                    ...
                    if (Array.isArray(dailyData) && dailyData.length === 3) {
                        phc.dataExtractorSimplePower(dailyData[0], dailyData[1], dailyData[2], phc.averagePeriodPowerObj, true);
                    }

                    for (let pType in phc.powerTypeMap) {
                        if (phc.powerTypeMap[pType].includes(dailyData[1])) { //dailyData[1] is powerName
                            if (!phc.powerTypeData[pType]) {
                                phc.powerTypeData[pType] = {};
                            }

                            if (!phc.powerTypeData[pType][dailyData[0]]) {
                                phc.powerTypeData[pType][dailyData[0]] = {};
                            }

                            for (let i in dailyData[2]) {
                                let timeData = dailyData[2][i];
                                let powerTypeTimeData = phc.powerTypeData[pType][dailyData[0]];
                                if (!powerTypeTimeData[timeData[0]]) {
                                    powerTypeTimeData[timeData[0]] = timeData;
                                } else {
                                    powerTypeTimeData[timeData[0]][1] += timeData[1];
                                    powerTypeTimeData[timeData[0]][2] += timeData[2];
                                }
                            }

                            for (let date in phc.powerTypeData[pType]) {
                                let arr = [];
                                for (let time in phc.powerTypeData[pType][date]) {
                                    arr.push(phc.powerTypeData[pType][date][time]);
                                }
                                phc.dataExtractorSimplePower(date, pType, arr, phc.averagePeriodPowerObj);
                            }
                        }
                    }
                }
...
//                    console.log(dailyData);
            ...
            phc.dataExtractorSimplePowerSorting(true);
...
//        if(!phc.reqSeries[])
        ...
        for (let i = 0; i < phc.newDaysArr.length; i++) {
            ...
            let dataArr = phc.rawDataBuff[newDay];
            let powerTypeDataBuf = phc.dataBuff[newDay];

            for (let j = 0; j < dataArr.length; j++) {
                let data = dataArr[j];
                for (var powerName in psManager.calcPowerList) {
                    let expression = JSON.parse(psManager.calcPowerList[powerName].expression);
                    if (expression.powerType !== 'other') {

                        phc.addPowerReqSeries(phcUtil.getReadablePowerName(expression.powerType));

                        let power = psManager.calcExpresion(expression, data, '-powerW');
                        if (power !== null && power !== undefined) {
                            powerTypeDataBuf[j][phcUtil.getReadablePowerName(expression.powerType) + '-powerW'] = power;
                        }
                        let energy = psManager.calcExpresion(expression, data, '-energyWh');
                        if (energy !== null && energy !== undefined) {
                            powerTypeDataBuf[j][phcUtil.getReadablePowerName(expression.powerType) + '-energyWh'] = energy;
                        }
                    }
                }
            }
    ...
    dataExtractorSimplePower: function (dateStr, powerName, dailyData, averagePeriodObj, isRawData) {

        //To not add raw data to the chart
        if (!isRawData) {
        ...
        if (Array.isArray(dailyData)) {
            ...
            if (!isRawData) {
                ...
                arr = phc.tempDataBuff[dateStr] = phc.tempDataBuff[dateStr] || [];
            } else {
                phc.knownSeriesPower[powerName] = phc.knownSeriesPower[powerName] || true;
                arr = phc.tempRawDataBuff[dateStr] = phc.tempRawDataBuff[dateStr] || [];
    ...
    dataExtractorSimplePowerSorting: function (isRawData) {
        ...
        if (isRawData) {
            tempDataBuff = phc.tempRawDataBuff;
            dataBuff = phc.rawDataBuff;
        } else {
            tempDataBuff = phc.tempDataBuff;
            dataBuff = phc.dataBuff;

        }

        for (var dateStr in tempDataBuff) {
            ...
            let daylyDataArr = tempDataBuff[dateStr];
            let destArrObj = {};
...
var phcUtil = {
    ...
    getReadablePowerName: function (pt) {
        switch (pt) {
            case "pv":
                {
                    return "PV";
