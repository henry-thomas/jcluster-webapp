...
var gaugeEnergy = {
    ...
    updateGaugeLevel: function (name) {
        ...
        if (char !== undefined) {
            ...
            if (!gaugeEnergy.status[name]) {
                char.labels[1].text('--%');
                gaugeEnergy.setCharLabel(char, 1, '--%');
            } else {
    ...
    updateGaugeLevelOld: function (name) {
        var char = gaugeEnergy.gaugeArr[name];
        if (char !== undefined) {
            var value;
            if (!gaugeEnergy.status[name]) {
                ...
                if (char.curLevel > char.futureLevel && (char.curLevel - char.futureLevel) > 1) {
                    var diff = char.curLevel - char.futureLevel;
                    char.curLevel -= diff / 2;
                    char.startDuration = 1 - ((diff / 130));
                } else if (char.curLevel < char.futureLevel && (char.futureLevel - char.curLevel) > 1) {
                    var diff = char.futureLevel - char.curLevel;
                    char.curLevel += diff / 5;
                    char.startDuration = 1.1 - ((diff / 150));
                }

                value = char.curLevel;
            }
            if (value <= 10) {
                char.axes[0].bands[2].setEndValue(30);
                char.axes[0].bands[1].setEndValue(10);
                char.axes[0].bands[0].setEndValue(value);
            } else if (value > 10 && value <= 30) {
                char.axes[0].bands[2].setEndValue(30);
                char.axes[0].bands[1].setEndValue(value);
                char.axes[0].bands[0].setEndValue(10);
            } else if (value > 30) {
                char.axes[0].bands[2].setEndValue(value);
                char.axes[0].bands[1].setEndValue(30);
                char.axes[0].bands[0].setEndValue(10);
            }

            if (!gaugeEnergy.status[name]) {
                char.labels[0].text('--%');
                char.arrows[0].setValue(0);
                gaugeEnergy.setCharLabel(char, 0, '--%');
            } else {
                gaugeEnergy.setCharLabel(char, 0, value.toFixed(0) + '%');
                char.arrows[0].setValue(value);
            }

            char.validateNow();
        }
    },
    createGauge: function (name) {
//        console.log('call- createGauge name: ' + name + ' capacity: ');
        gaugeEnergy.gaugeTimer[name] = setTimeout(gaugeEnergy.generateTimerCallbackFn(name), 5000);
    ...
    setGaugeValueOld: function (name, capacity, maxCapacity, power, isCharge, time) {
        var char = gaugeEnergy.gaugeArr[name];
        if (char !== undefined) {
            gaugeEnergy.timerReset(name);
            if (!gaugeEnergy.status[name]) {
            ...
            if (!isNaN(capacity) && !isNaN(maxCapacity) && !isNaN(power) && !isNaN(isCharge) && !isNaN(time)) {
                ...
                if (capacity !== undefined && maxCapacity !== undefined) {
                    ...
                    char.futureLevel = value;
                    power = Number(power);
                    char.allLabels[1].text = Number(power / 1E3).toFixed(2) + ' kW';
                    //text for charge or discharge

                    if (isCharge) {
                        char.allLabels[2].text = 'Remaining charge time:';
                    } else {
                        char.allLabels[2].text = 'Remaining discharge time:';
                    }
                    time = Number(time);
                    if (time === 0) {
                        char.allLabels[3].text = '-d --h:--min';
                    } else {
                        char.allLabels[3].text = gaugeEnergy.getTimeFromSeconds(time);
                    }
                }
            }
        }
...
//        console.log('TIMER RESET ' + timerID + ' id: ' + storageName);
        ...
        gaugeEnergy.gaugeTimer[storageName] = setTimeout(gaugeEnergy.generateTimerCallbackFn(storageName), 5000);
    },
    ...
    getTimeFromSeconds: function (tSec) {
        ...
        var mt = moment(tSec);
        var month = mt.get('month');
        var days = mt.get('date') - 1;
        var result;
        if (days > 0) {
            if (month > 0) {
                result = mt.format("MM[m] - DD[d] HH[h] mm[m]");
            } else {
                result = mt.format("DD[d] - HH[h] mm[m]");
            }
        } else {
            result = mt.format("HH[h] mm[m]");
        }
        return result;
    },
    ...
    var graph = {
        ...
        "allLabels": [
            ...
            },
            {
                ...
                "color": "#909090",
                "id": "timeRemainLabel",
                "text": "Remaining dis/charge time:",
                "y": "85%"
            },
            {
                "align": "center",
