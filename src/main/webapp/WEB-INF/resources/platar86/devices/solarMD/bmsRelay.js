...
//        console.log(newIo);
    ...
    setChanges: function (fieldName, value) {
        if (devManager.getSelected() !== undefined) {
            ...
            var arr = devManager.getSelected().getParam().ioArrChanges;
            var arrNormal = devManager.getSelected().getParam().ioArr;
    ...
    getRelayValueFromObj: function (fieldName) {
        ...
        var io = param.ioArr[bmsRelay.currentIO];
        var ioChanges = param.ioArrChanges[bmsRelay.currentIO];
//        console.log(JSON.stringify(io, null, 2));
        if (io !== undefined) {
            if (ioChanges === undefined) {
                ioChanges = {};
            }
            if (ioChanges[fieldName] !== undefined) {
                return ioChanges[fieldName];
            }
            return io[fieldName];
        }
    },
...
//        #define IO_COMP_LOGIC_OFF_FIRST 1
    ...
    onDataReveived: function () {
        console.log('on data');
        var ioState = devManager.getSelected().getData().ioState[bmsRelay.currentIO];
        var ioPendingState = devManager.getSelected().getData().ioPendingState[bmsRelay.currentIO];
    ...
    onParamReveived: function (dev) {
        ...
        if (param.ioArr !== undefined) {
            if (param.ioArrChanges === undefined) {
                param.ioArrChanges = [];
            }
...
var sendRelayIoInstruction = function () {
    if (devManager.getSelected() !== undefined
            && devManager.getSelected().getParam().ioArrChanges !== undefined) {

        var arr = devManager.getSelected().getParam().ioArrChanges[bmsRelay.currentIO];
        ...
        mu.sendWsMessage(message,
                function (devMessage) { //on success
                    ...
                    delete devManager.getSelected().getParam().ioArrChanges[bmsRelay.currentIO][fieldName];
                    devManager.getSelected().getParam().ioArr[bmsRelay.currentIO][fieldName] = value;
