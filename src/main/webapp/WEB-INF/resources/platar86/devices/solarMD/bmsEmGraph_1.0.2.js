...
bmsEmGraph.chartID = 'chartdiv';
bmsEmGraph.show = false;
...
/**
 * Loading on the graph the retrieved data from the database for the provided serial number and range of dates
 * @param {type} data
 * @returns
 */
bmsEmGraph.loadRecordsRangeFromDB = function (data) {
    if (!bmsEmGraph.show) {
        document.getElementById(bmsEmGraph.chartID).style.display = 'block';
        bmsEmGraph.show = !bmsEmGraph.show;
    }
    
    bmsEmGraph.graphData.length = 0;
    for (var i = 0; i < data.length; i++) {
        bmsEmGraph.addGraphRecord(null, data[i]);
    }
    bmsEmGraph.chart.validateData();
};

bmsEmGraph.clear = function () {
    if (bmsEmGraph.show) {
        document.getElementById(bmsEmGraph.chartID).style.display = 'none';
        bmsEmGraph.show = !bmsEmGraph.show;
    }
...
var devManager = devManager || null;
...
bmsEmGraph.initialize =  function(id) {
    bmsEmGraph.chartID = id || 'chartdiv';
    bmsEmGraph.chart = AmCharts.makeChart(bmsEmGraph.chartID,
    {
        "type": "serial",
        "categoryField": "date",
