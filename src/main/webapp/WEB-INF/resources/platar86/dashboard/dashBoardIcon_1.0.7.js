...
/* global SVG, energyServiceBcReq, powerBcReq, dashMan, mainUtils, devTempData, eStorageTemp, mu */
...
;
dashMan.onStorageReceived(function (d) {
    if (devTempData.dSwitch.state === 1) {
        dComp.updateComponent(d, 'storage');
    }
});

dashMan.onPowerReceived(function (d) {
    if (devTempData.dSwitch.state === 1) {
...
var dComp = {
    ...
    getBatChargePower: function (manager) {
        if (dashMan.eStorageArr.dataAvailable) {
            let p = dashMan.eStorageArr.sumData.powerW;
            if (p > 0) {
                return p;
            }
            return 0;
        }
        if (dashMan.getPowerStorageCharge().dataAvailable) {
            return  dashMan.getPowerStorageCharge().sumData.powerW;
        }
        if (dashMan.getPowerStorageDischarge().dataAvailable && dashMan.getPowerStorageDischarge().sumData.powerW > 0) {
            return 0;
        }

        var pv = dashMan.getPowerPv();
        var grid = dashMan.getPowerGridOut();
        var load = dashMan.getPowerLoad();
    ...
    getBatDischargePower: function (manager) {
        ...
        if (dashMan.getPowerStorageDischarge().dataAvailable) {
            return  dashMan.getPowerStorageDischarge().sumData.powerW;
        }
        if (dashMan.getPowerStorageCharge().dataAvailable && dashMan.getPowerStorageCharge().sumData.powerW > 0) {
            return 0;
        }
    ...
    updateComponent: function (manager, eventType) {
        if (dashMan.eStorageArr.dataAvailable) {
            ...
            var storage = dashMan.eStorageArr.sumData;
...
//            console.log(storage.remainingTimeSign);
            ...
            if (storage.powerW < 0) {
                dComp.bat.levelBar.setMaxLevel(dashMan.eStorageArr.sumData.reatedDischargePowerW);
                dComp.bat.levelBar.setLevel(storage.powerW * -1);

                dComp.bat.dotAnimBatDischOrange.setMaxLevel(dashMan.eStorageArr.sumData.reatedDischargePowerW);
                dComp.load.dotAnimLoadOrange.setMaxLevel(dashMan.eStorageArr.sumData.reatedDischargePowerW);
                dComp.bat.dotAnimBatDischOrange.setLevel(storage.powerW * -1);
                dComp.load.dotAnimLoadOrange.setLevel(storage.powerW * -1);
            } else {
                dComp.bat.levelBar.setMaxLevel(dashMan.eStorageArr.sumData.reatedChargePowerW);
                dComp.bat.levelBar.setLevel(storage.powerW);

                dComp.bat.dotAnimBatDischOrange.setMaxLevel(dashMan.eStorageArr.sumData.reatedChargePowerW);
                dComp.load.dotAnimLoadOrange.setMaxLevel(dashMan.eStorageArr.sumData.reatedChargePowerW);
...
//                dComp.load.dotAnimLoadOrange.setLevel(storage.powerW );
            ...
            if (!isNaN(capkWh)) {
                ...
                mu.formatDashboardValue(storage.powerW, dComp.bat.labels.power, 'W');

                if (ratedCapkWh >= 1000) {
                    SVG.get('batCapInKwhLabel').plain((capkWh / 1000).toFixed(1) + ' (' + (ratedCapkWh / 1000).toFixed(1) + ')');
                    SVG.get('batCapInKwhUnitLabel').plain('MWh');
                }
                if (ratedCapkWh >= 1000000) {
                    SVG.get('batCapInKwhLabel').plain((capkWh / 1000000).toFixed(1) + ' (' + (ratedCapkWh / 1000000).toFixed(1) + ')');
                    SVG.get('batCapInKwhUnitLabel').plain('GWh');
                }
        ...
        var pvObj = dashMan.getPowerPv();
        var gridObj = dashMan.getPowerGridOut();
        var loadObj = dashMan.getPowerLoad();
        ...
        if (pSum.powerW !== undefined) {
            ...
            mu.formatDashboardValue(pSum.powerW, dComp.pv.labels.power, 'W', SVG.get('pvPowerUnitLabel'));
            mu.formatDashboardValue(pSum.dailyEnergyWh, dComp.pv.labels.dEnergy, 'Wh');
            mu.formatDashboardValue(pSum.energyWh, dComp.pv.labels.tEnergy, 'Wh');
...
//        SVG.get('pvBalloonGroup').style('opacity', 1);
        if (gSum.powerW !== undefined) {
            mu.formatDashboardValue(gSum.powerW, dComp.grid.labels.power, 'W', SVG.get('gridPowerUnitLabel'));
            mu.formatDashboardValue(gSum.dailyEnergyWh, dComp.grid.labels.dEnergy, 'Wh');
            mu.formatDashboardValue(gSum.energyWh, dComp.grid.labels.tEnergy, 'Wh');
        ...
        if (lSum.powerW !== undefined) {
            mu.formatDashboardValue(lSum.powerW, dComp.load.labels.power, 'W', SVG.get('pvPowerUnitLabel-6'));
            mu.formatDashboardValue(lSum.dailyEnergyWh, dComp.load.labels.dEnergy, 'Wh');
            mu.formatDashboardValue(lSum.energyWh, dComp.load.labels.tEnergy, 'Wh');
...
dashMan.onPowerInit(function (dManager, powerTypes) {
    if (dashMan.initFlags.initPServiceBc === true) {
        if (dashMan.powerTypes.gridExport !== undefined && Object.keys(dashMan.powerTypes.gridExport).length > 0) {
            SVG.get('gridBalloonGroup').style('opacity', 0.5);
        }
        if (dashMan.powerTypes.load !== undefined && Object.keys(dashMan.powerTypes.load).length > 0) {
            SVG.get('loadBalloonGroup').style('opacity', 0.5);
        }
        if (dashMan.powerTypes.load !== undefined && Object.keys(dashMan.powerTypes.pv).length > 0) {
            SVG.get('pvBalloonGroup').style('opacity', 0.5);
        }
    }
});