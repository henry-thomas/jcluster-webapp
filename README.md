...
@JcTimeout set timeout for specific call. Timeout exception is thrown if no response is received after timeout milliseconds since request has been sent.

@JcBroadcst sends message to all available instances

@JcBroadcast combined with @JcFilter sends message to all instance containing specific filter match
